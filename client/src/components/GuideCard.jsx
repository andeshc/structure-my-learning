...
export function displayGuideTitle(title) {
  return title.replace(/\s+(roadmap|guide)$/i, '');
}

export function guideTags(guide) {
...
export function GuideIllustration({ guide, index, title }) {
...
// extraMenu: array of { label, icon, onSelect, destructive }
export default function GuideCard({ guide, index, onDelete, extraMenu = [] }) {
  ...
  return (
    <article
      className="flex flex-col overflow-hidden rounded-xl border border-slate-200 transition-all hover:border-teal-200 hover:shadow-[0_0_0_3px_rgba(15,118,110,0.07)]"
    ...
    >
      <div className="relative aspect-[3/2] overflow-hidden border-b border-slate-100">
        ...
        <div ref={menuRef} className="absolute right-2 top-2">
          ...
          {isMenuOpen && (
            <div className="absolute right-0 top-9 z-50 w-44 rounded-lg border border-slate-200 bg-white p-1" role="menu">
              {extraMenu.map((item) => (
                <button
                  key={item.label}
                  className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-semibold transition-colors hover:bg-slate-50 ${item.destructive ? 'text-red-700 hover:bg-red-50' : 'text-slate-700'}`}
                  type="button"
                  role="menuitem"
                  onClick={() => {
                    setIsMenuOpen(false);
      ...
      <div className="flex-1 p-4">
        ...
        {guide.collections && guide.collections.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {guide.collections.map((col) => (
              <Link
                key={col.id}
                to={`/collections/${col.id}`}
                onClick={(e) => e.stopPropagation()}
                className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500 hover:bg-teal-50 hover:text-teal-700"
              >
                <Folder size={10} />
                {col.name}
              </Link>
